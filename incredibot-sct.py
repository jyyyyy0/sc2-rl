...
class IncrediBot(BotAI):  # inhereits from BotAI (part of BurnySC2)
    async def on_step(self, iteration: int):  # on_step is a method that is called every step of the game.
        # map_x_length = int(self.game_info.map_size[0])
        # map_y_length = int(self.game_info.map_size[1])
        no_action = True
        while no_action:
            try:
            ...
            try:
                ...
                if not found_something:
                    ...
                    for nexus in self.townhalls:
                        ...
                        worker_count = len(self.workers.closer_than(25, nexus))
                        if worker_count < 21:  # 16+3+3
                            if nexus.is_idle and self.can_afford(UnitTypeId.PROBE) and self.supply_left > 1:
        ...
        # 1: build stargate (or up to one) (evenly)
            ...
            try:
                ...
                for nexus in self.townhalls:
                    if not self.units(UnitTypeId.GATEWAY).closer_than(10, nexus).exists:
                    ...
                    if not self.units(UnitTypeId.CYBERNETICSCORE).closer_than(10, nexus).exists:
                    ...
                    if not self.units(UnitTypeId.STARGATE).closer_than(10, nexus).exists:
        ...
        # 5: voidray flee (back to base)
            ...
            if self.units(UnitTypeId.STALKER).amount > 0:
                for sk in self.units(UnitTypeId.STALKER):
                    sk.attack(self.start_location)

        # init the map
        map = np.zeros((185, 192, 3), dtype=np.uint8)
        ...
        # draw our structures:
        for our_structure in self.units:
            # if it's a nexus:
        ...
        for vespene in self.state.units.vespene_geyser:
        ...
        for our_unit in self.units:
            ...
            elif our_unit.type_id == UnitTypeId.STALKER:
                pos = our_unit.position
                c = [75, 75, 75]
                fraction = our_unit.health / our_unit.health_max if our_unit.health_max > 0 else 0.0001
                map[math.ceil(pos.y)][math.ceil(pos.x)] = [int(fraction * i) for i in c]
        ...
        try:
            ...
            for stalker in self.units(UnitTypeId.STALKER):
                if stalker.is_attacking and stalker.target_in_range:
                    if self.state.enemy_units.closer_than(8, stalker) or self.known_enemy_structures.closer_than(8,
                                                                                                                 stalker):
                        reward += 0.010
                        attack_count += 0.5
            ...
            for voidray in self.units(UnitTypeId.VOIDRAY):
                ...
                if voidray.is_attacking and voidray.target_in_range:
                    if self.state.enemy_units.closer_than(8, voidray) or self.known_enemy_structures.closer_than(8,
                                                                                                                 voidray):
...
result = run_game(  # run_game is a function that runs the game.
    ...
    [Bot(Race.Protoss, IncrediBot()),  # runs our coded bot, protoss race, and we pass our bot object
     Computer(Race.Zerg, Difficulty.CheatMoney)],  # runs a pre-made computer agent, zerg race, with a hard difficulty.
